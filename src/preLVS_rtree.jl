...
export loadDB, flatten, alignEvents, generate_graph, runLVS, runLVS_wo_log, rectMerge
...
function runLVS_wo_log(runset::Union{String, Dict})
    ...
    # 7. Check Connections
    error_log, error_cnt, hash_rect, nets_visited, cgraph = check_connections_bfs(cgraph, pinNodes, hash_rect, equiv_net_sets)
...
function rectMerge(runset::Union{String, Dict})
    ...
    # 8. merging metals using convex hull --> valid only to laygo-generated
    _mdata, _vdata  = convex_hull(cgraph, hash_rect)
    # # 8. Create Error Log File
    # create_error_log_file(error_log, error_cnt, log_dir, libname, cellname, hash_rect, nets_visited, djs)
    # println("error log file created")
    mdata, vdata    = load_rects(_mdata, _vdata, config_data)

    return mdata, vdata, cgraph, hash_rect
end
...
@setup_workload begin
    ...
    @compile_workload begin
        ...
        mdata, vdata, cgraph, hash_rect = rectMerge(sample_data_path)
        println("   Running rectMerge compile script complete")
    end
    # ### `@setup_workload` 블록의 끝은 `@compile_workload` 블록 *뒤*에 와야 합니다. ###
    println("Finished preLVS_rtree precompile workload setup.")
end # @setup_workload 블록의 끝
# --- 워크로드 끝 ---

end # module preLVS_rtree
